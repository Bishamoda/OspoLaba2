	...
	var trigger=0, name, age, sex, education, mathematic, asu, cutting, programming,count=0; // обьявляем переменные которые понадобятся в ходе работы
		...
		if (trigger==1) {
			if(document.getElementById("asu").checked ==true) {count++;}
			if(document.getElementById("mathematic").checked ==true) {count++;}
			if(document.getElementById("cutting").checked ==true) {count++;}
			if(document.getElementById("programming").checked ==true) {count++;}
		if (document.getElementById("education").checked ==true && count>=2)
		{
			count=0;
